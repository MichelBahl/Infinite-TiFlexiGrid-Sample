...
function init(_table) {
        ...
        if(__parentSymbol.getApiName() == 'Ti.UI.ScrollView'){
        //	__parentSymbol.addEventListener('dragEnd', dragEndScrollView);
        }
...
function dragEndScrollView(e){

	dragEndPosition	= e.source.contentOffset.y;
}
...
function onScroll(e) {
		...
		if(e.source.getApiName() == 'Ti.UI.ScrollView'){

	        Ti.API.info("scrollViewPosition  + : " + scrollViewPosition +e.source.contentOffset.y);
	        Ti.API.info("childs  + : " + e.source.children.length);
		   	triggerLoad = 	(e.source.contentOffset.y + scrollViewPosition + e.source.size.height) >  (e.source.size.height * triggerCounts);
        }else{
        	
        //	!$.is.getVisible() && $.is.setVisible(true);
	        ...
	        if (OS_ANDROID) {
	                ...
	                // last pixel shown
	                triggerLoad = (position && e.contentOffset.y > position) && (e.contentOffset.y + e.size.height > e.contentSize.height - 200);
	
					 //Ti.API.info("triggerLoad  + : " + triggerLoad);
